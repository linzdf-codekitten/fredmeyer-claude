# QFC extract snippets

JavaScript and Python snippets used by the `qfc-export` skill. Pass the JS to `chrome-devtools:evaluate_script` after navigating; the page is React/Next.js, so allow it to hydrate first.

QFC runs on the same Kroger platform as Fred Meyer. Page structure, selectors, and API endpoints are identical — only the domain (`qfc.com`) differs.
...
Run after navigating to qfc.com:
...
() => {
  ...
  document.querySelectorAll('a[href*="/mypurchases/detail/"]').forEach(a => {
    ...
    orders.push({ id, date, type, url: 'https://www.qfc.com' + a.getAttribute('href') });
  });
  return orders;
}
```
...
async () => {
  ...
  if (!aside) { localStorage.setItem('qfc_' + ID, ''); return 0; }
  ...
  aside.querySelectorAll('li').forEach(li => {
    ...
    const url  = 'https://www.qfc.com' + href;
  ...
  localStorage.setItem('qfc_' + ID, rows.join('\n'));
  return rows.length;
}
```
...
```javascript
() => {
  const keys = Object.keys(localStorage).filter(k => k.startsWith('qfc_'));
  const result = {};
  keys.forEach(k => { result[k.slice(4)] = localStorage.getItem(k); });
...
Run after writing all `/tmp/qfc-order-*.csv` files. Use Python (not bash `sort`) so quoted CSV fields are handled correctly.

```python
import csv, io, glob

all_rows = []
for path in glob.glob('/tmp/qfc-order-*.csv'):
    with open(path) as f:
        for line in f:
            line = line.strip()
            if line:
...
with open('qfc-purchases.csv', 'w') as f:
...
Alternative to DOM extraction. Returns clean JSON per order, but Akamai rate-limits direct fetches after ~5 calls. Use only when the order count is small. Requires active qfc.com cookies.
